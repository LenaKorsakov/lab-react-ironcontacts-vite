...
const sortByName = (nameA, nameB) => nameA.localeCompare(nameB, undefined, { sensitivity: "base" })
...
const getRandomInt = (min, max) => {
  ...
  while(arrOfInt.includes(randNum)) {
...
function App() {
  ...
  const handleButtonSortByName = () => {
    const sortedContacts = [...contacts].sort((persA, persB) => sortByName(persA.name, persB.name));
  ...
  return (
    <div className="App">
      ...
      <button className='btn' onClick={handleButtonClick}>Add Random Contact</button>
      <button className='btn' onClick={handleButtonSortByPopularity}>Sort By Popularity</button>
      <button className='btn' onClick={handleButtonSortByName}>Sort By Name</button>
      </div>
      <table>
        <thead>
        <tr>
        ...
        <tbody>
        {
          contacts.map((pers) => {
           return <tr key={pers.id}>
              ...
              <td>
                <button className="delete" onClick={() => handleDeleteButton(pers.id)}>Delete</button>
              </td>
            </tr>
          })
        }
